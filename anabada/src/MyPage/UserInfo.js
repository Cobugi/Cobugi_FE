...
const UserInfo = () => {
    ...
    const myRegister_lend = lend.filter((product => product.registeredUserId === curEmail));
    const myRegister_seek = seek.filter((product => product.registeredUserId === curEmail));
    ...
    const mySeek_lend = lend.filter(product =>
        product.reservedUsersInfo.some(reservedUser =>
          reservedUser.reservedUser === curEmail
        )
    );
    //2. seek 각 원소에 registeredUserId가 내 id이고 state가 1인 경우
    const mySeek_seek = seek.filter((product => product.registeredUserId === curEmail));  
    //3. 내가 빌린 물건 최종
    ...
    const myLend_lend = lend.filter((product => product.registeredUserId === curEmail && product.state === '1'));
    //2. seek 각 원소에 lendingUserId가 나인 경우
    const myLend_seek = seek.filter((product => product.lendingUserId === curEmail));
    //3. 내가 빌려준 물건 최종
